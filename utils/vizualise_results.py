...
def vizualise_result_semseg(name_model: str, dataloader: str, device: str, num_epoch: int):
    ...
    list_names = ['UNetModel', 'AttentionUnetModel', 'SegFormer', 'LinSegFormer', 'LongSegFormer']
    count_equals = 0
    ...
    if name_model == 'UNetModel':
    ...
    elif name_model == 'AttentionUnetModel':
        ...
        model = LinSegFormer(in_channels=3,
                            widths=[64, 128, 256, 512],
                            depths=[3, 4, 6, 3],
                            all_num_heads=[1, 2, 4, 8],
                            patch_sizes=[7, 3, 3, 3],
